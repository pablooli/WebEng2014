...
public class TaskOperationsFile {
	...
	public static synchronized void addTask(Tarea tarea) {
	...
	public static synchronized void deleteTarea(int idTask) {
		Gson gson = new Gson();
	...
	// delete an task.
		...
		while (!encontrado && indice<listTask.size()){
			Tarea tarea = listTask.getListaTarea().get(indice);			if (tarea.getId()==idTask) {
